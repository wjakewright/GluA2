...
from organize_roi_features import extract_roi_values, extract_roi_pixel_values
from compute_pixel_lifetime import compute_pixel_lifetime


def analyze_mean_lifetime(
    mouse_list,
    save=True,
):
    """Function to handle the preprocessing of aligned images and extract pixels and calcualte
    mean lifetime for the different atlas ROIs
...
def analyze_pixel_lifetime(
    mouse_list,
    modality="lifetime",
    save=True,
):
    """Function to handle the preprocessing of aligned images and extract pixels for
    the different atlas ROIs and calculate lifetime for each pixel
    ...
    for mouse in mouse_list:
        ...
        for i, file_name in enumerate(base_fnames):
            ...
            # Extract lifetime for each ROI
            roi_df = extract_roi_pixel_values(
                lifetime_image, current_atlas, mouse, modality
            )
        ...
        if save:
            ...
            if not os.path.isdir(save_path):
                os.makedirs(save_path)
            save_name = os.path.join(save_path, f"{mouse}_pixel_data_{modality}.pickle")
            concatenated_df.to_pickle(save_name)
